...
async function askGenesis(question) {
      ...
      return d.answer || "Ask me anything about Nexus."
}
...
function GenesisQA() {
  ...
  const sessionTopics = React.useRef([])

  function speak(text) {
    if (!window.speechSynthesis) return
    window.speechSynthesis.cancel()
    setSpeaking(true)
    const mode = text.includes("?") ? "companion" : (text.includes("build")||text.includes("create")) ? "insight" : "neutral"
    const modes = {neutral:{r:0.95,p:1.00},companion:{r:0.92,p:1.06},insight:{r:0.90,p:1.10}}
    const utt = new SpeechSynthesisUtterance(text)
    utt.rate = modes[mode].r; utt.pitch = modes[mode].p; utt.volume = 1
    utt.onend = () => setSpeaking(false)
    utt.onerror = () => setSpeaking(false)
    const vs = window.speechSynthesis.getVoices()
    const v = vs.find(v => v.name === "Google UK English Female") || vs.find(v => v.name === "Samantha") || vs.find(v => v.lang && v.lang.startsWith("en"))
    if (v) utt.voice = v
    window.speechSynthesis.speak(utt)
  }
...
return (<section style={{padding:"60px 24px",textAlign:"center",borderTop:"1px solid rgba(255,255,255,0.04)"}}><div style={{maxWidth:660,margin:"0 auto"}}><div style={{color:"#252545",fontFamily:"monospace",fontSize:11,letterSpacing:4,marginBottom:16}}>LIVE Q&A</div><h2 style={{fontSize:"clamp(22px,4vw,36px)",fontWeight:900,margin:"0 0 12px",color:"#dde0ff"}}>Ask Genesis Anything</h2><p style={{color:"#505070",marginBottom:32,lineHeight:1.7,fontSize:14}}>She knows everything about Nexus.</p><div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap",justifyContent:"center"}}><input value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>e.key==="Enter"&&ask()} placeholder="Ask anything..." style={{flex:1,minWidth:220,padding:"13px 16px",background:"rgba(5,12,30,0.9)",border:"1px solid rgba(0,229,255,0.3)",borderRadius:10,color:"#dde0ff",fontFamily:"monospace",fontSize:13}}/><button onClick={()=>ask()} disabled={busy||!q.trim()} style={{padding:"13px 24px",background:busy||!q.trim()?"rgba(10,15,35,0.8)":"linear-gradient(135deg,#00e5ff,#0066ff)",color:busy||!q.trim()?"#1a1a40":"#000",border:"none",borderRadius:10,fontFamily:"monospace",fontSize:13,fontWeight:700,cursor:busy?"default":"pointer"}}>{busy?"...":"ASK"}</button></div>{(ans||busy)&&(<div style={{background:"rgba(5,12,30,0.85)",border:"1px solid rgba(0,229,255,0.2)",borderRadius:14,padding:"20px 24px",textAlign:"left",marginBottom:16}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}><div style={{width:26,height:26,borderRadius:"50%",background:"radial-gradient(circle at 35% 30%,#00ffff,#0066cc)",boxShadow:"0 0 8px rgba(0,229,255,0.5)",flexShrink:0}}/><span style={{color:"#00e5ff",fontFamily:"monospace",fontSize:11,letterSpacing:3}}>{speaking?"GENESIS SPEAKING...":"GENESIS"}</span></div>{busy?<div style={{color:"#505070",fontFamily:"monospace",fontSize:13}}>Genesis is thinking...</div>:<div style={{color:"#c8ccee",fontSize:14,lineHeight:1.85,fontStyle:"italic"}}>{ans}</div>}</div>)}{!ans&&!busy&&(<div style={{display:"flex",gap:8,flexWrap:"wrap",justifyContent:"center"}}>{["What is Nexus?","Who is Kevin Albert?","Is it really free?","What does Genesis do?","What is the Orb?"].map(s=>(<button key={s} onClick={()=>ask(s)} style={{padding:"8px 14px",background:"rgba(0,229,255,0.05)",border:"1px solid rgba(0,229,255,0.15)",borderRadius:20,color:"#505070",fontFamily:"monospace",fontSize:11,cursor:"pointer"}}>{s}</button>))}</div>)}</div></section>);
}
